...
} from "@/features/companies/queries/sla.queries.js";
...
const EditScoreModal = ({ review, companyFilter = null, onClose }) => {
  ...
  const companyId = review?.company_id
    ? String(review.company_id)
    : review?.location?.company_id
    ? String(review.location.company_id)
    : companyFilter
  ...
  const locationId = review?.location_id
    ...
    : review?.location?.id
    ? String(review.location.id)
    : null;

  // 1. Fetch Company SLA
  const { data: companySlaData, isLoading: loadingCompanySla } = useCompanySlaConfig(
    companyId,
    Boolean(companyId)
  );

  // 2. Fetch Washroom SLA override
  const { data: washroomSlaData, isLoading: loadingWashroomSla } = useWashroomSlaConfig(
    locationId,
    Boolean(locationId)
  );

  const isCompanySlaActive = Boolean(companySlaData?.enabled);
  const companyThreshold = companySlaData?.configuration?.threshold_score != null
    ? Number(companySlaData.configuration.threshold_score)
    : null;

  const washroomSlaConfig = washroomSlaData?.configuration || washroomSlaData;
  const isWashroomSlaActive = Boolean(washroomSlaData?.enabled || washroomSlaConfig?.enabled || washroomSlaConfig?.is_active);
  const washroomThreshold = washroomSlaConfig?.threshold_score != null
    ? Number(washroomSlaConfig.threshold_score)
    : null;
        ...
        <form onSubmit={handleSave} className="p-5 space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-500 font-medium">Original / Saved Score:</span>
              <span className="font-bold font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                {(review?.original_score ?? review?.score ?? 0).toFixed(1)} / 10
              </span>
            </div>

            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
              New Hygiene Score (0.0 – 10.0)
            </label>
            <div className="flex items-center gap-3">
              <input
                type="number"
                ...
                disabled={isUpdating}
                className="w-24 px-3 py-2 text-sm font-bold rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              />
              <input
                type="range"
                min="0"
              ...
              />
              <span className={`text-sm font-bold min-w-10 text-right ${getScoreColor(numericScore)}`}>
                {isValidScore ? numericScore.toFixed(1) : "--"}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-purple-50/70 dark:bg-purple-950/30 border border-purple-200/80 dark:border-purple-900/60">
            ...
            <label className="relative inline-flex items-center cursor-pointer shrink-0">
              <input
                type="checkbox"
                checked={triggerEscalation}
                onChange={(e) => setTriggerEscalation(e.target.checked)}
                disabled={isUpdating}
          ...
          <div className="flex justify-end gap-2.5 pt-2 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              type="submit"
              disabled={isUpdating || !isValidScore}
              className="px-4 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 rounded-lg shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
            >
              <Save size={14} />
              {isUpdating ? "Updating Score..." : "Save & Update Score"}
            </button>
          </div>
        </form>
      </div>
